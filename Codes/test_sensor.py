...
from utils import get_file_name_without_ext, get_sensor_csv_path, get_bag_info_path, get_new_model_path,\
    ...
    get_info_path, get_default_model_path, get_sensor_abnormality_path
import json
import math
...
def test_sensor_values(model_path, scaling_factor, shifting_factor, min_sigma):
    ...
    session = tf.compat.v1.Session(config=config)  # config=config must
    with session:
        ...
        df.to_csv(r'temp_sensor_result.csv', index=False, header=True)"""
...
"""def test_sensor_values(model_path, scaling_factor, shifting_factor, min_sigma):
    process = multiprocessing.Process(target=test_sensor_values_fun, args=(model_path, scaling_factor,
            shifting_factor, min_sigma,))
    process.start()
    process.join()
    dataframe = pd.read_csv('temp_sensor_result.csv')
    p = dataframe['p']
    q = dataframe['q']
    r = dataframe['r']
    s = dataframe['s']
    time_axis = dataframe['time_axis']
    return p, q, r, s, time_axis"""
...
class RealTimeTestObj():
    def __init__(self, model_path, scaling_factor, shifting_factor, min_sigma):
        ...
        #session = tf.Session(config=config)  # config=config must
